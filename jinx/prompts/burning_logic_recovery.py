...
class BurningLogicRecoveryPrompt(BasePrompt):
    """Prompt provider for the *burning_logic_recovery* prompt.

    This variant focuses on error‑recovery scenarios while keeping the same
    tag conventions as the main prompt. Subclassing :class:`BasePrompt`
    enables future templating.
    """
    ...
    def load(self) -> str:
        return (
            ...
            "- If uncertainty exists, ask a single clarifying question using <cpython_question_{key}> (wrapped in print(...)).\\n"
            ...
            "  <cpython_question_{key}>...<\/python_question_{key}>  (use if you need clarification)\\n"
            "  <cpython_{key}>...<\/python_{key}>                  (use for the final executable fix)\\n\\n"
            "Rules:\\n"
            "- Use only these tags: <machine_{key}>, <cpython_question_{key}>, <cpython_{key}>.\\n"
            ...
            "</machine_{key}>\\n"
            "<cpython_{key}>\\n"
            "# final fix code (or use <cpython_question_{key}> if clarification is needed)\\n"
            "</python_{key}>\\n"
        )


# Register the prompt using a lambda that returns the loaded string
register_prompt("burning_logic_recovery", lambda: BurningLogicRecoveryPrompt().load())
