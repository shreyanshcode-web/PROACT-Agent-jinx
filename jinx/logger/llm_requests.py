...
async def write_llm_request_dump(
    target_dir: str,
    kind: str,
    instructions: str,
    input_text: str,
    ...
    """Write a single LLM request dump to a uniquely named file.
...
async def write_llm_response_append(path: str, kind: str, output_text: str) -> bool:
    """Append a response section to an existing LLM request dump file.

    Parameters
    ----------
    path : str
        Path returned by `write_llm_request_dump`.
    kind : str
